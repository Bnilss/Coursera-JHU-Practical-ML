...
The data for this project comes from this original source: [Here](http://groupware.les.inf.puc-rio.br/har). If you use the document you create for this class for any purpose please cite them as they have been very generous in allowing their data to be used for this kind of assignment.  
...
```{r warning=FALSE, error=FALSE}
setwd("C:/Users/pc1/Desktop/REnvirements/RProjects")
```  

Downloads the dataset to the `data` folder.  
```{r warning=FALSE, error=FALSE}
...
## Cleaning Data  
Now, we will clean the dataset and get rid of observations with missing values as well as some usless variables.  
...
2. Removing some columns of the dataset that do not contribute to the accelerometer measurements.  
```{r warning=FALSE, error=FALSE}
...
3. Removing columns with messing values.  
```{r warning=FALSE, error=FALSE}
